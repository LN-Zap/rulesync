...
var SkillsProcessor = class extends DirFeatureProcessor {
  ...
  async loadRulesyncDirs() {
    ...
    const localDirPaths = await findFilesByGlobs((0, import_node_path70.join)(rulesyncSkillsDirPath, "*"), { type: "dir" });
    const localDirNames = localDirPaths.map((path4) => (0, import_node_path70.basename)(path4)).filter((name) => name !== ".curated");
    ...
    const curatedDirPath = (0, import_node_path70.join)(rulesyncSkillsDirPath, ".curated");
    let curatedSkills = [];
    if (await directoryExists(curatedDirPath)) {
      const curatedDirPaths = await findFilesByGlobs((0, import_node_path70.join)(curatedDirPath, "*"), { type: "dir" });
      const curatedDirNames = curatedDirPaths.map((path4) => (0, import_node_path70.basename)(path4));
      ...
      const curatedRelativeDirPath = (0, import_node_path70.join)(paths.relativeDirPath, ".curated");
...
async function readLockFile(params) {
  const lockPath = (0, import_node_path113.join)(params.baseDir, RULESYNC_SOURCES_LOCK_RELATIVE_FILE_PATH);
...
// src/lib/sources.ts
var FETCH_CONCURRENCY_LIMIT2 = 10;
...
async function getLocalSkillNames(baseDir) {
  const skillsDir = (0, import_node_path114.join)(baseDir, RULESYNC_SKILLS_RELATIVE_DIR_PATH);
  const names = /* @__PURE__ */ new Set();
  if (!await directoryExists(skillsDir)) {
    return names;
  }
  const dirPaths = await findFilesByGlobs((0, import_node_path114.join)(skillsDir, "*"), { type: "dir" });
  for (const dirPath of dirPaths) {
    const name = (0, import_node_path114.basename)(dirPath);
...
async function checkLockedSkillsExist(curatedDir, skillNames) {
  ...
  for (const name of skillNames) {
    if (!await directoryExists((0, import_node_path114.join)(curatedDir, name))) {
      return false;
    }
  }
  return true;
...
async function fetchSource(params) {
  ...
  const curatedDir = (0, import_node_path114.join)(baseDir, RULESYNC_CURATED_SKILLS_RELATIVE_DIR_PATH);
  ...
  const semaphore = new import_promise2.Semaphore(FETCH_CONCURRENCY_LIMIT2);
  const fetchedSkills = {};
  if (locked) {
    for (const prevSkill of lockedSkillNames) {
      const prevDir = (0, import_node_path114.join)(curatedDir, prevSkill);
  ...
  for (const skillDir of filteredDirs) {
    ...
    for (const file of files) {
      ...
      const localFilePath = (0, import_node_path114.join)(curatedDir, skillDir.name, relativeToSkill);
      checkPathTraversal({
        relativePath: relativeToSkill,
        intendedRootDir: (0, import_node_path114.join)(curatedDir, skillDir.name)
      });
      const content = await withSemaphore(
        semaphore,
...
async function listCommands() {
  const commandsDir = (0, import_node_path115.join)(process.cwd(), RULESYNC_COMMANDS_RELATIVE_DIR_PATH);
  try {
    ...
    const commands = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: (0, import_node_path115.join)(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getCommand({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: (0, import_node_path115.join)(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename),
...
async function putCommand({
  ...
  try {
    ...
    const isUpdate = existingCommands.some(
      (command2) => command2.relativePathFromCwd === (0, import_node_path115.join)(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteCommand({ relativePathFromCwd }) {
  ...
  const filename = (0, import_node_path115.basename)(relativePathFromCwd);
  const fullPath = (0, import_node_path115.join)(process.cwd(), RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename);
  try {
...
var commandTools = {
  listCommands: {
    name: "listCommands",
    description: `List all commands from ${(0, import_node_path115.join)(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
...
async function getIgnoreFile() {
  const ignoreFilePath = (0, import_node_path116.join)(process.cwd(), RULESYNC_AIIGNORE_RELATIVE_FILE_PATH);
  try {
...
async function deleteIgnoreFile() {
  const aiignorePath = (0, import_node_path116.join)(process.cwd(), RULESYNC_AIIGNORE_RELATIVE_FILE_PATH);
  const legacyIgnorePath = (0, import_node_path116.join)(process.cwd(), RULESYNC_IGNORE_RELATIVE_FILE_PATH);
  try {
...
async function getMcpFile() {
  try {
    ...
    const relativePathFromCwd = (0, import_node_path117.join)(
...
async function putMcpFile({ content }) {
  ...
  try {
    ...
    const fullPath = (0, import_node_path117.join)(baseDir, relativeDirPath, relativeFilePath);
    ...
    await ensureDir((0, import_node_path117.join)(baseDir, relativeDirPath));
    await writeFileContent(fullPath, content);
    const relativePathFromCwd = (0, import_node_path117.join)(relativeDirPath, relativeFilePath);
    return {
      relativePathFromCwd,
      content: rulesyncMcp.getFileContent()
    };
...
async function deleteMcpFile() {
  try {
    const baseDir = process.cwd();
    const paths = RulesyncMcp.getSettablePaths();
    const recommendedPath = (0, import_node_path117.join)(
      baseDir,
      paths.recommended.relativeDirPath,
      paths.recommended.relativeFilePath
    );
    const legacyPath = (0, import_node_path117.join)(baseDir, paths.legacy.relativeDirPath, paths.legacy.relativeFilePath);
...
async function listRules() {
  const rulesDir = (0, import_node_path118.join)(process.cwd(), RULESYNC_RULES_RELATIVE_DIR_PATH);
  try {
    ...
    const rules = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: (0, import_node_path118.join)(RULESYNC_RULES_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getRule({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: (0, import_node_path118.join)(RULESYNC_RULES_RELATIVE_DIR_PATH, filename),
...
async function putRule({
  ...
  try {
    ...
    const isUpdate = existingRules.some(
      (rule2) => rule2.relativePathFromCwd === (0, import_node_path118.join)(RULESYNC_RULES_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteRule({ relativePathFromCwd }) {
  ...
  const filename = (0, import_node_path118.basename)(relativePathFromCwd);
  const fullPath = (0, import_node_path118.join)(process.cwd(), RULESYNC_RULES_RELATIVE_DIR_PATH, filename);
  try {
    await removeFile(fullPath);
    return {
      relativePathFromCwd: (0, import_node_path118.join)(RULESYNC_RULES_RELATIVE_DIR_PATH, filename)
...
var ruleTools = {
  listRules: {
    name: "listRules",
    description: `List all rules from ${(0, import_node_path118.join)(RULESYNC_RULES_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
...
function extractDirName(relativeDirPathFromCwd) {
  const dirName = (0, import_node_path119.basename)(relativeDirPathFromCwd);
  if (!dirName) {
...
async function listSkills() {
  const skillsDir = (0, import_node_path119.join)(process.cwd(), RULESYNC_SKILLS_RELATIVE_DIR_PATH);
  try {
    const skillDirPaths = await findFilesByGlobs((0, import_node_path119.join)(skillsDir, "*"), { type: "dir" });
    const skills = await Promise.all(
      skillDirPaths.map(async (dirPath) => {
        const dirName = (0, import_node_path119.basename)(dirPath);
        ...
        try {
          ...
          const frontmatter = skill.getFrontmatter();
          return {
            relativeDirPathFromCwd: (0, import_node_path119.join)(RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName),
            frontmatter
          };
        } catch (error) {
...
async function putSkill({
  ...
  try {
    ...
    const isUpdate = existingSkills.some(
      (skill2) => skill2.relativeDirPathFromCwd === (0, import_node_path119.join)(RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName)
    );
    ...
    const skillDirPath = (0, import_node_path119.join)(process.cwd(), RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName);
    await ensureDir(skillDirPath);
    const skillFilePath = (0, import_node_path119.join)(skillDirPath, SKILL_FILE_NAME);
    ...
    for (const file of otherFiles) {
      ...
      const filePath = (0, import_node_path119.join)(skillDirPath, file.name);
      const fileDir = (0, import_node_path119.join)(skillDirPath, (0, import_node_path119.dirname)(file.name));
...
var skillTools = {
  listSkills: {
    name: "listSkills",
    description: `List all skills from ${(0, import_node_path119.join)(RULESYNC_SKILLS_RELATIVE_DIR_PATH, "*", SKILL_FILE_NAME)} with their frontmatter.`,
...
async function listSubagents() {
  const subagentsDir = (0, import_node_path120.join)(process.cwd(), RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH);
  try {
    ...
    const subagents = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: (0, import_node_path120.join)(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getSubagent({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: (0, import_node_path120.join)(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename),
...
async function putSubagent({
  ...
  try {
    ...
    const isUpdate = existingSubagents.some(
      (subagent2) => subagent2.relativePathFromCwd === (0, import_node_path120.join)(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteSubagent({ relativePathFromCwd }) {
  ...
  const filename = (0, import_node_path120.basename)(relativePathFromCwd);
  const fullPath = (0, import_node_path120.join)(process.cwd(), RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename);
  try {
    await removeFile(fullPath);
    return {
      relativePathFromCwd: (0, import_node_path120.join)(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename)
...
var subagentTools = {
  listSubagents: {
    name: "listSubagents",
    description: `List all subagents from ${(0, import_node_path120.join)(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
