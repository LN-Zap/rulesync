...
var SkillsProcessor = class extends DirFeatureProcessor {
  ...
  async loadRulesyncDirs() {
    ...
    const localDirPaths = await findFilesByGlobs(join69(rulesyncSkillsDirPath, "*"), { type: "dir" });
    const localDirNames = localDirPaths.map((path4) => basename18(path4)).filter((name) => name !== ".curated");
    ...
    const curatedDirPath = join69(rulesyncSkillsDirPath, ".curated");
    let curatedSkills = [];
    if (await directoryExists(curatedDirPath)) {
      const curatedDirPaths = await findFilesByGlobs(join69(curatedDirPath, "*"), { type: "dir" });
      const curatedDirNames = curatedDirPaths.map((path4) => basename18(path4));
      ...
      const curatedRelativeDirPath = join69(paths.relativeDirPath, ".curated");
...
import { basename as basename24, join as join113 } from "path";
...
async function readLockFile(params) {
  const lockPath = join112(params.baseDir, RULESYNC_SOURCES_LOCK_RELATIVE_FILE_PATH);
...
async function getLocalSkillNames(baseDir) {
  const skillsDir = join113(baseDir, RULESYNC_SKILLS_RELATIVE_DIR_PATH);
  const names = /* @__PURE__ */ new Set();
  if (!await directoryExists(skillsDir)) {
    return names;
  }
  const dirPaths = await findFilesByGlobs(join113(skillsDir, "*"), { type: "dir" });
  for (const dirPath of dirPaths) {
    const name = basename24(dirPath);
...
async function checkLockedSkillsExist(curatedDir, skillNames) {
  ...
  for (const name of skillNames) {
    if (!await directoryExists(join113(curatedDir, name))) {
      return false;
    }
  }
  return true;
...
async function fetchSource(params) {
  ...
  const curatedDir = join113(baseDir, RULESYNC_CURATED_SKILLS_RELATIVE_DIR_PATH);
  ...
  const semaphore = new Semaphore2(FETCH_CONCURRENCY_LIMIT2);
  const fetchedSkills = {};
  if (locked) {
    for (const prevSkill of lockedSkillNames) {
      const prevDir = join113(curatedDir, prevSkill);
  ...
  for (const skillDir of filteredDirs) {
    ...
    for (const file of files) {
      ...
      const localFilePath = join113(curatedDir, skillDir.name, relativeToSkill);
      checkPathTraversal({
        relativePath: relativeToSkill,
        intendedRootDir: join113(curatedDir, skillDir.name)
      });
      const content = await withSemaphore(
        semaphore,
...
async function listCommands() {
  const commandsDir = join114(process.cwd(), RULESYNC_COMMANDS_RELATIVE_DIR_PATH);
  try {
    ...
    const commands = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: join114(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getCommand({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: join114(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename),
...
async function putCommand({
  ...
  try {
    ...
    const isUpdate = existingCommands.some(
      (command2) => command2.relativePathFromCwd === join114(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteCommand({ relativePathFromCwd }) {
  ...
  const filename = basename25(relativePathFromCwd);
  const fullPath = join114(process.cwd(), RULESYNC_COMMANDS_RELATIVE_DIR_PATH, filename);
  try {
...
var commandTools = {
  listCommands: {
    name: "listCommands",
    description: `List all commands from ${join114(RULESYNC_COMMANDS_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
...
async function getIgnoreFile() {
  const ignoreFilePath = join115(process.cwd(), RULESYNC_AIIGNORE_RELATIVE_FILE_PATH);
  try {
...
async function deleteIgnoreFile() {
  const aiignorePath = join115(process.cwd(), RULESYNC_AIIGNORE_RELATIVE_FILE_PATH);
  const legacyIgnorePath = join115(process.cwd(), RULESYNC_IGNORE_RELATIVE_FILE_PATH);
  try {
...
async function getMcpFile() {
  try {
    ...
    const relativePathFromCwd = join116(
...
async function putMcpFile({ content }) {
  ...
  try {
    ...
    const fullPath = join116(baseDir, relativeDirPath, relativeFilePath);
    ...
    await ensureDir(join116(baseDir, relativeDirPath));
    await writeFileContent(fullPath, content);
    const relativePathFromCwd = join116(relativeDirPath, relativeFilePath);
    return {
      relativePathFromCwd,
      content: rulesyncMcp.getFileContent()
    };
...
async function deleteMcpFile() {
  try {
    const baseDir = process.cwd();
    const paths = RulesyncMcp.getSettablePaths();
    const recommendedPath = join116(
      baseDir,
      paths.recommended.relativeDirPath,
      paths.recommended.relativeFilePath
    );
    const legacyPath = join116(baseDir, paths.legacy.relativeDirPath, paths.legacy.relativeFilePath);
...
// src/mcp/rules.ts
import { basename as basename26, join as join117 } from "path";
import { z as z59 } from "zod/mini";
var maxRuleSizeBytes = 1024 * 1024;
var maxRulesCount = 1e3;
async function listRules() {
  const rulesDir = join117(process.cwd(), RULESYNC_RULES_RELATIVE_DIR_PATH);
  try {
    ...
    const rules = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: join117(RULESYNC_RULES_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getRule({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: join117(RULESYNC_RULES_RELATIVE_DIR_PATH, filename),
...
async function putRule({
  ...
  try {
    ...
    const isUpdate = existingRules.some(
      (rule2) => rule2.relativePathFromCwd === join117(RULESYNC_RULES_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteRule({ relativePathFromCwd }) {
  ...
  const filename = basename26(relativePathFromCwd);
  const fullPath = join117(process.cwd(), RULESYNC_RULES_RELATIVE_DIR_PATH, filename);
  try {
    await removeFile(fullPath);
    return {
      relativePathFromCwd: join117(RULESYNC_RULES_RELATIVE_DIR_PATH, filename)
...
var ruleTools = {
  listRules: {
    name: "listRules",
    description: `List all rules from ${join117(RULESYNC_RULES_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
...
// src/mcp/skills.ts
import { basename as basename27, dirname as dirname3, join as join118 } from "path";
...
async function listSkills() {
  const skillsDir = join118(process.cwd(), RULESYNC_SKILLS_RELATIVE_DIR_PATH);
  try {
    const skillDirPaths = await findFilesByGlobs(join118(skillsDir, "*"), { type: "dir" });
    const skills = await Promise.all(
      skillDirPaths.map(async (dirPath) => {
        ...
        try {
          ...
          const frontmatter = skill.getFrontmatter();
          return {
            relativeDirPathFromCwd: join118(RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName),
            frontmatter
          };
        } catch (error) {
...
async function putSkill({
  ...
  try {
    ...
    const isUpdate = existingSkills.some(
      (skill2) => skill2.relativeDirPathFromCwd === join118(RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName)
    );
    ...
    const skillDirPath = join118(process.cwd(), RULESYNC_SKILLS_RELATIVE_DIR_PATH, dirName);
    await ensureDir(skillDirPath);
    const skillFilePath = join118(skillDirPath, SKILL_FILE_NAME);
    ...
    for (const file of otherFiles) {
      ...
      const filePath = join118(skillDirPath, file.name);
      const fileDir = join118(skillDirPath, dirname3(file.name));
...
var skillTools = {
  listSkills: {
    name: "listSkills",
    description: `List all skills from ${join118(RULESYNC_SKILLS_RELATIVE_DIR_PATH, "*", SKILL_FILE_NAME)} with their frontmatter.`,
...
import { basename as basename28, join as join119 } from "path";
import { z as z61 } from "zod/mini";
var maxSubagentSizeBytes = 1024 * 1024;
var maxSubagentsCount = 1e3;
async function listSubagents() {
  const subagentsDir = join119(process.cwd(), RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH);
  try {
    ...
    const subagents = await Promise.all(
      mdFiles.map(async (file) => {
        try {
          ...
          return {
            relativePathFromCwd: join119(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, file),
            frontmatter
          };
        } catch (error) {
...
async function getSubagent({ relativePathFromCwd }) {
  ...
  try {
    ...
    return {
      relativePathFromCwd: join119(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename),
...
async function putSubagent({
  ...
  try {
    ...
    const isUpdate = existingSubagents.some(
      (subagent2) => subagent2.relativePathFromCwd === join119(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename)
    );
...
async function deleteSubagent({ relativePathFromCwd }) {
  ...
  const filename = basename28(relativePathFromCwd);
  const fullPath = join119(process.cwd(), RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename);
  try {
    await removeFile(fullPath);
    return {
      relativePathFromCwd: join119(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, filename)
...
var subagentTools = {
  listSubagents: {
    name: "listSubagents",
    description: `List all subagents from ${join119(RULESYNC_SUBAGENTS_RELATIVE_DIR_PATH, "*.md")} with their frontmatter.`,
